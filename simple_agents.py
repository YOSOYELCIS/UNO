...
class MDPAgent(Player):
    ...
    State:
        - agent hand size
        - next player hand size
        - top discard color/value
        - playable cards
    ...
    Reward:
        - high reward for winning
        - reward for reducing hand size
        - reward for attacking next player
        - reward for keeping good future colors
        - penalty for wasting Wild/Draw Four too early
    """
    ...
    def q_value(self, card: Card, g_state: GameState) -> float:
        ...
        hand_size = len(self.hand)
        next_player = g_state.players[
            (g_state.turn + g_state.direction) % g_state.player_count
        ]
        next_hand_size = len(next_player.hand)

        # 1. Winning move
        if hand_size == 1:
            reward += 1000

        # 2. Basic reward for playing any card
        reward += 10

        # 3. Prefer stronger cards when opponent is close to winning
        if next_hand_size <= 2:
            if card.value == Value.DRAW_FOUR:
                reward += 120
            elif card.value == Value.DRAW_TWO:
        ...
        if card.value == Value.DRAW_FOUR:
            ...
            reward += NUM_VALS.get(card.value, 0)

        # 5. Preserve future playability
        if card.color != Color.BLACK:
            same_color_count = sum(
                1 for c in self.hand
                if c.color == card.color
            ...
            reward += same_color_count * 4

            if same_color_count == 1:
                reward += 10

        # 6. Prefer matching by value
        top_card = g_state.deck.discard[0]
        if card.value == top_card.value:
            reward += 8

        # 7. Avoid wasting wilds early
        if hand_size >= 6:
            if card.value == Value.DRAW_FOUR:
                reward -= 25
            elif card.value == Value.WILD:
